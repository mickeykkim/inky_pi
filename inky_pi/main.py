...
from inky_pi.configs import (T_LDB_TOKEN, T_NUM, T_STATION_FROM, T_STATION_TO,
                             T_WSDL, W_API_KEY, W_EXCLUDE, W_LATITUDE,
                             W_LONGITUDE)
...
def main() -> None:
    ...
    train_data: TrainBase = HuxleyOpenLDBWS(T_STATION_FROM, T_STATION_TO,
                                            T_NUM)
    train_data2: TrainBase = OpenLDBWS(T_STATION_FROM, T_STATION_TO, T_NUM,
                                       T_WSDL, T_LDB_TOKEN)
    weather_data: WeatherBase = OpenWeatherMap(W_LATITUDE, W_LONGITUDE,
                                               W_EXCLUDE, W_API_KEY)

    train_data2.fetch_train(T_NUM)
